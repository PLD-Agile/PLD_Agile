    ...
    def branch_and_bound(
        ...
        if not unvisited:
            if self._graph.is_arc(current_vertex, 0):
                if (
                    current_cost + self._graph.get_cost(current_vertex, 0)
                    < self.__best_sol_cost
                ):
                    self.__best_sol_cost = current_cost + self._graph.get_cost(
                        current_vertex, 0
                    )
        elif (
            current_cost + self.bound(current_vertex, unvisited) < self.__best_sol_cost
        ):
            ...
            for next_vertex in it:
                ...
                self.branch_and_bound(
                    ...
                    + self._graph.get_cost(current_vertex, next_vertex),
                )
