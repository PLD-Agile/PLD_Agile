...
from PyQt6.QtWidgets import QLabel, QVBoxLayout, QWidget, QTableWidget
...
from src.views.ui import Text, TextSize, Button, Callout
...
class ReadDeliveryMan(QWidget):
    ...
    def __init__(self):
        ...
        layout.addWidget(Callout('S\'il y a des livreurs sur des tournées, vous ne pourrez pas les supprimer.'))
    ...
    def __update_table(self, delivery_men: Dict[UUID, DeliveryMan], computed_tours: List[Tour]):
        ...
        for delivery_man in delivery_men.values():
            ...
            delete_button.clicked.connect(lambda _, delivery_man=delivery_man: self.__delete_delivery_man(delivery_man))
            
            if delivery_man.id in computed_tours:
                delete_button.setEnabled(False)
            
            self.__table.setCellWidget(row_index, 0, QLabel(delivery_man.name))
            self.__table.setCellWidget(row_index, 1, QLabel(self.__availabilities_to_string(delivery_man.availabilities)))
    ...
    def __availabilities_to_string(self, availabilities: List[int]) -> str:
        ...
        for availability in availabilities:
            if len(joined_availabilities) == 0 or joined_availabilities[-1][1] != availability - 1:
                joined_availabilities.append((availability, availability))
            else:
                joined_availabilities[-1] = (joined_availabilities[-1][0], availability)
        
        return ', '.join([f'{start}:00 - {end + 1}:00' for start, end in joined_availabilities])
